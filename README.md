...
При первом запуске backend автоматически заполняет базу банком вопросов из `src/main/resources/question-bank.tsv`: 16 тестов по 21 вопросу. В файл перенесены только вопросы, варианты ответа, пары соответствия и правильные ответы; пояснения из исходного документа не импортируются, потому что они генерируются AI во время прохождения.
